...
private fun getSigningConfigForAGP4(agpVersion: String, project: Project, variantScope: VariantScope): SigningConfig {
    ...
    var buildTypeData: Any? = null
    buildTypes.forEach { (type, data) ->
        if (flavor.endsWith(type as String, true)) {
            buildTypeData = data
        }
    }
    if (buildTypeData == null) {
        throw GradleException("get buildType failed for $flavor $buildTypes")
    }
